...
class SplineCubica():
    ...
    def __init__(self, x:list[float], y:list[float], condicao = 'natural', valores = [0,0]):
        ...
        if isinstance(condicao, str):                                           # Verifica se condicao é uma string
            if condicao not in ['natural', 'fixado']:                           # Verifica se condicao é 'natural' ou 'fixado'
                raise ValueError("condicao deve ser 'natural' ou 'fixado'.")
            
        self.x = x
        self.y = y
        self.condicao = condicao
        self.valores = valores
        self.coeficientes = sistema_equacoes.solucao(x, y, condicao, valores)
